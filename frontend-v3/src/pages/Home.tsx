...
export default function Home() {
	...
	const [tripFilter, setTripFilter] = useState<'all' | 'upcoming' | 'ongoing' | 'past'>('all');
	...
	const upcomingCount = ownedTrips.filter((t) => getTripStatus(t) === 'upcoming').length;
	const ongoingCount = ownedTrips.filter((t) => getTripStatus(t) === 'ongoing').length;
	const pastCount = ownedTrips.filter((t) => getTripStatus(t) === 'past').length;
		...
		<Container maxWidth="xl">
			...
			<Box sx={{ mb: 5 }}>
				...
				>
					...
					>
						<Tab icon={<DashboardIcon />} label={t('myTrips')} iconPosition="start" sx={{ gap: 1.5 }} />
						<Tab icon={<Analytics />} label={t('statistics')} iconPosition="start" sx={{ gap: 1.5 }} />
					</Tabs>
				</Paper>
			</Box>
			...
			{activeTab === 0 && (
				<Box>
						...
						<Box>
							...
							{trips && (
								<Typography variant="body2" color="text.secondary">
									{filteredOwnedTrips.length} {filteredOwnedTrips.length === 1 ? t('trip') : t('trips')}
								</Typography>
							)}
						</Box>
						<Button
						...
						>
							<Chip
								...
								sx={{
									...
									'&:hover': {
										bgcolor: tripFilter === 'all' ? 'primary.dark' : 'action.hover',
							...
							<Chip
								...
								sx={{
									...
									cursor: 'pointer',
									'&:hover': {
										bgcolor: tripFilter === 'upcoming' ? 'primary.main' : 'action.hover',
									},
								}}
							/>
							<Chip
								...
								sx={{
									...
									cursor: 'pointer',
									'&:hover': {
										bgcolor: tripFilter === 'ongoing' ? 'success.main' : 'action.hover',
									},
								}}
							/>
							<Chip
								...
								sx={{
									...
									cursor: 'pointer',
									'&:hover': {
										bgcolor: tripFilter === 'past' ? 'action.selected' : 'action.hover',
									},
								}}
							/>
						</Stack>
							...
							<Stack direction="row">
								...
								<Button
									...
									sx={{
										...
										'&:hover': {
											bgcolor: viewMode === 'compact' ? 'primary.dark' : 'action.hover',
										},
									}}
								>
									<ViewComfy />
					...
					{trips && filteredOwnedTrips.length === 0 && filteredSharedTrips.length === 0 && (
						<Fade in timeout={800}>
							<Paper
								elevation={0}
								sx={{
							...
							>
								...
								<Typography variant="body1" color="text.secondary" mb={4} sx={{ maxWidth: 500 }}>
									{tripFilter === 'all' ? t('startPlanningFirstTrip') : `${t('youDontHaveAny')} ${t(tripFilter)} ${t('trips')}`}
								</Typography>
								{tripFilter === 'all' && (
									<Button
										variant="contained"
								...
								<Fade in timeout={500 + index * 100}>
									<Card
										sx={{
											...
											borderColor: 'divider',
											'&:hover': {
												transform: viewMode !== 'compact' ? 'translateY(-4px)' : 'translateY(-2px)',
												...
												>
													...
													<Box flex={1}>
														<Stack direction="row" spacing={1} alignItems="center" mb={0.5}>
															{viewMode === 'compact' && <Luggage sx={{ fontSize: 20, color: 'primary.main' }} />}
															<Typography
																...
																sx={{
																	...
																	display: '-webkit-box',
																	WebkitLineClamp: viewMode === 'compact' ? 1 : 2,
															...
															{viewMode !== 'list' && (
																<Chip
																	label={
																		getTripStatus(trip) === 'upcoming' ? t('upcoming') : getTripStatus(trip) === 'ongoing' ? t('ongoing') : t('past')
																	}
																	size="small"
																	color={getTripStatus(trip) === 'upcoming' ? 'primary' : getTripStatus(trip) === 'ongoing' ? 'success' : 'default'}
																	sx={{
																		height: 24,
																		fontSize: '0.75rem',
																		fontWeight: 600,
												...
												{trip.startDate && trip.endDate && (
													...
													>
														<Stack direction="row" spacing={1} alignItems="center">
															...
															/>
															<Typography variant={viewMode === 'compact' ? 'caption' : 'body2'} fontWeight={500} color="text.primary">
													...
													<Stack direction="row" spacing={1.5} sx={{ mt: 'auto' }}>
														{trip.flights?.length > 0 && (
															<Stack direction="row" spacing={0.5} alignItems="center">
																<Flight sx={{ fontSize: 14, color: 'primary.main' }} />
														...
														{trip.attractions?.length > 0 && (
															<Stack direction="row" spacing={0.5} alignItems="center">
																<Attractions sx={{ fontSize: 14, color: 'success.main' }} />
																<Typography variant="caption" fontWeight={600} color="text.secondary">
						...
						<Box sx={{ mt: 6 }}>
							...
							>
								<Box>
									...
									<Typography variant="body2" color="text.secondary">
										{filteredSharedTrips.length} {filteredSharedTrips.length === 1 ? t('trip') : t('trips')}
									</Typography>
								</Box>
							</Stack>
																...
																<Stack direction="row" spacing={1} alignItems="center" mb={0.5}>
																	...
																	{viewMode !== 'list' && (
																		<Chip
																			...
																			size="small"
																			color={
																				getTripStatus(trip) === 'upcoming' ? 'primary' : getTripStatus(trip) === 'ongoing' ? 'success' : 'default'
																			}
																			sx={{
																				height: 24,
																				fontSize: '0.75rem',
