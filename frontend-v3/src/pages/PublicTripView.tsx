...
export default function PublicTripView() {
  ...
  const loadTrip = async () => {
    try {
      ...
      setError(err.response?.data?.error || t('errorLoadingTripDetails'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || t('errorSubmittingRequest'));
    }
  };

  const handleShare = (platform: 'whatsapp' | 'email') => {
    const url = window.location.href;
    const text = `${t('comeToAmazingTrip')}! ${trip?.title} ${t('to')} ${trip?.destination}`;
          ...
          <Grid item xs={12} md={8}>
            <Paper sx={{ p: 3, mb: 3 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' }}>
                ...
                />
                {trip.status === 'full' && <Chip label={t('full')} color="error" />}
              </Box>
          ...
          <Grid item xs={12} md={4}>
            ...
            <Card sx={{ position: 'sticky', top: 20, mb: 3 }}>
              <CardContent>
                ...
                >
                  {trip.status === 'full' ? t('tripIsFull') : t('wantToRegister')}
                </Button>
              </CardContent>
            </Card>
