...
import { AutoAwesome, Close, LocationOn, Schedule, AttachMoney, Restaurant, Hotel, LocalActivity, ExpandMore, Add, Check } from '@mui/icons-material';
...
export default function AIItinerarySuggester({ trip, onSuggestionsApplied }: AIItinerarySuggesterProps) {
	const { t } = useLanguage();
	...
	const handleGenerateSuggestions = async () => {
		...
		try {
			...
			const response = await fetch(`${API_BASE_URL}/ai/suggest-itinerary`, {
				method: 'POST',
				headers: {
	...
	const handleToggleSuggestion = (dayIndex: number, suggestionIndex: number) => {
	...
	const handleApplySuggestions = async () => {
		...
		try {
			...
			suggestions.forEach((day, dayIndex) => {
				day.suggestions.forEach((suggestion, suggestionIndex) => {
					...
					if (selectedSuggestions.has(key)) {
						if (suggestion.type === 'attraction' || suggestion.type === 'activity' || suggestion.type === 'restaurant') {
							newAttractions.push({
								...
								type: suggestion.type === 'restaurant' ? 'restaurant' : 'attraction',
							});
			...
			for (const hotel of newHotels) {
				const hotelResponse = await fetch(`${API_BASE_URL}/trips/${trip.id}/hotels`, {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
						Authorization: `Bearer ${token}`,
				...
				if (!hotelResponse.ok) {
					...
					console.error('Failed to add hotel:', hotelResponse.status, errorText);
			...
			for (const attraction of newAttractions) {
				const attractionResponse = await fetch(`${API_BASE_URL}/trips/${trip.id}/attractions`, {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
						Authorization: `Bearer ${token}`,
				...
				if (!attractionResponse.ok) {
					...
					console.error('Failed to add attraction:', attractionResponse.status, errorText);
					throw new Error(`Failed to add attraction: ${attractionResponse.status}`);
				}
			}

			// Fetch updated trip
			const tripResponse = await fetch(`${API_BASE_URL}/trips/${trip.id}`, {
				headers: {
					Authorization: `Bearer ${token}`,
				},
			});
										...
										<Stack direction="row" spacing={1} sx={{ mt: 1 }}>
											{['relaxed', 'moderate', 'packed'].map((pace) => (
												<Chip
													...
													color={preferences.pace === pace ? 'primary' : 'default'}
													variant={preferences.pace === pace ? 'filled' : 'outlined'}
												/>
											))}
										</Stack>
									</Box>
							...
							<>
									...
									>
										<AccordionSummary expandIcon={<ExpandMore />}>
											<Stack direction="row" alignItems="center" gap={2}>
												...
												<Box>
													<Typography fontWeight={700}>{new Date(day.date).toLocaleDateString()}</Typography>
												...
												{day.suggestions.map((suggestion, suggestionIndex) => {
													...
													return (
														<Card
															...
															onClick={() => handleToggleSuggestion(dayIndex, suggestionIndex)}
														>
															<CardContent>
																<Stack direction="row" alignItems="flex-start" spacing={2}>
																	...
																	>
																		{isSelected ? <Check /> : getSuggestionIcon(suggestion.type)}
																	</Box>

																	<Box sx={{ flex: 1 }}>
																		<Stack direction="row" alignItems="center" spacing={1} mb={0.5}>
																			<Typography fontWeight={700}>{suggestion.name}</Typography>
																			<Chip label={t(suggestion.type)} size="small" sx={{ fontSize: '0.7rem' }} />
																		</Stack>
																		...
																		<Stack direction="row" spacing={2} flexWrap="wrap">
																			<Chip icon={<LocationOn />} label={suggestion.location} size="small" variant="outlined" />
																			{suggestion.suggestedTime && (
																				<Chip icon={<Schedule />} label={suggestion.suggestedTime} size="small" variant="outlined" />
																			)}
																			{suggestion.estimatedCost && (
																				<Chip icon={<AttachMoney />} label={`$${suggestion.estimatedCost}`} size="small" variant="outlined" />
																			)}
																		</Stack>
								...
								<Stack direction="row" spacing={2}>
									...
									<Button
										...
										sx={{ borderRadius: 3 }}
									>
										{loading ? t('applying') : `${t('addSelected')} (${selectedSuggestions.size})`}
									</Button>
								</Stack>
							</>
						)}
