...
import { Card, CardContent, Typography, Stack, Box, CircularProgress, Alert } from '@mui/material';
...
async function loadRealPrices(destination: string, checkIn: string, checkOut: string, origin: string) {
	try {
	...
	for (const day of aiData.itinerary) {
		for (const activity of day.activities) {
			if (activity.type === 'hotel' && activity.placeId) {
				...
				const prices = await loadRealPrices('Dubai', '2025-03-01', '2025-03-05', 'New York');
					...
					<CardContent>
						<Stack direction="row" justifyContent="space-between" alignItems="center">
							<Box>
								...
								<Typography variant="body2">${hotel.pricePerNight}/night</Typography>
							</Box>
					...
					<CardContent>
						<Stack direction="row" justifyContent="space-between" alignItems="center">
							<Box>
								...
								<Typography variant="body2">{flight.stops === 0 ? 'Direct' : `${flight.stops} stops`}</Typography>
							</Box>
...
function CompactHotelDisplay({ hotel }: { hotel: Hotel }) {
	return (
		<Stack direction="row" spacing={1} alignItems="center">
			...
			<BookingButton bookingLink={hotel.bookingLink} price={hotel.price} type="hotel" affiliate={true} variant="chip" size="small" />
		</Stack>
	);
}

export { HotelCard, loadRealPrices, createAITripWithBookings, TripDetailsWithBookings, CompactHotelDisplay };
