...
interface ChatContextSelectorProps {
	...
	onCreateChat: (contextType: string, contextId: string, participants: Participant[], metadata: any) => void;
}

export default function ChatContextSelector({ open, onClose, contextType, onCreateChat }: ChatContextSelectorProps) {
	const { user } = useAuth();
	...
	const fetchContextData = async () => {
		try {
			...
			if (contextType === 'trip') {
				...
				const response = await fetch(`${API_BASE_URL}/quicket/my-items`, {
					headers: {
						Authorization: `Bearer ${token}`,
					},
				});
	...
	const handleContextSelect = async (context: any) => {
		...
		if (contextType === 'trip' && context) {
			...
			try {
				const token = localStorage.getItem('authToken');
				const response = await fetch(`${API_BASE_URL}/trips/${context._id}`, {
					headers: {
						Authorization: `Bearer ${token}`,
					},
				});
					...
					const tripParticipants: Participant[] = [
						{
							...
							name: trip.userName || trip.userEmail?.split('@')[0] || 'Trip Creator',
							role: 'owner',
						},
	...
	const handleCreateChat = async () => {
		...
		if ((contextType === 'friend_group' || contextType === 'direct') && !groupName.trim()) {
		...
		try {
			...
			onCreateChat(contextType, selectedContext._id || 'new', finalParticipants, metadata);
	...
	return (
		<Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
					...
					<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
						{contextType === 'trip' && (
							<Autocomplete
								...
								renderInput={(params) => <TextField {...params} label={getContextLabel()} />}
							/>
						)}
						...
						{contextType === 'quicket_item' && (
							<Autocomplete
								...
								getOptionLabel={(option) => `${option.title || option.name} (${option.type})`}
								value={selectedContext}
						...
						{(contextType === 'friend_group' || contextType === 'direct') && (
							<TextField label="Group Name" value={groupName} onChange={(e) => setGroupName(e.target.value)} fullWidth />
						)}
						...
						<Box>
							...
							<Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mb: 1 }}>
								{participants.map((p) => (
									<Chip
										key={p.email}
										label={`${p.name} (${p.role})`}
										onDelete={p.userId !== user?.id && contextType !== 'trip' ? () => handleRemoveParticipant(p.email) : undefined}
										size="small"
							...
							{(contextType === 'friend_group' || contextType === 'direct') && (
								<Autocomplete
									...
									renderInput={(params) => <TextField {...params} label="Add participant by email" helperText="Press Enter to add" size="small" />}
								/>
							)}
						</Box>
					</Box>
			...
			<DialogActions>
				...
				<Button onClick={handleCreateChat} variant="contained" disabled={loading || !selectedContext}>
					Create Chat
				</Button>
			</DialogActions>
		</Dialog>
