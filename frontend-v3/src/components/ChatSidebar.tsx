...
export default function ChatSidebar({ open, onClose, onChatSelect, onNewChat }: ChatSidebarProps) {
	const { user } = useAuth();
	...
	const handleChatClick = (chatId: string, contextType: string) => {
		onChatSelect(chatId, contextType);
	};
	...
	const handleLokaClick = () => {
		...
		onChatSelect('loka-ai-chat', 'ai_assistant');
	...
	const filterChats = () => {
		...
		if (searchQuery) {
			filtered = filtered.filter((chat) => {
				...
				// Search in metadata
				const metadataMatch =
					chat.metadata.itemTitle?.toLowerCase().includes(searchLower) || chat.metadata.tripName?.toLowerCase().includes(searchLower);
	...
	const getTabUnreadCount = (contextType: 'direct' | 'trip' | 'quicket_item') => {
		return chats.filter((chat) => chat.contextType === contextType).reduce((sum, chat) => sum + getUnreadCount(chat), 0);
	};
									...
									>
										<ListItemAvatar>
											<Badge badgeContent={unreadCount} color="error" overlap="circular">
												<Avatar src={chatImage || undefined} sx={{ width: 50, height: 50 }}>
													{chat.contextType === 'quicket_item' && <QuicketIcon />}
													{chat.contextType === 'trip' && <TripIcon />}
													{chat.contextType === 'direct' && getOtherParticipant(chat)?.name[0]?.toUpperCase()}
												</Avatar>
											</Badge>
										</ListItemAvatar>
										...
										<ListItemText
											...
											primary={
												...
												>
													...
													>
														...
														<Chip
															size="small"
															label={chat.contextType === 'quicket_item' ? 'Quicket' : chat.contextType === 'trip' ? 'Trip' : 'Friend'}
															color={chat.contextType === 'quicket_item' ? 'primary' : chat.contextType === 'trip' ? 'info' : 'default'}
											...
											secondary={
												<Box>
													...
													{chat.contextType === 'quicket_item' && chat.status === 'pending' && (
														<Chip label="Pending" size="small" color="warning" sx={{ mt: 0.5, height: 20 }} />
													)}
												</Box>
											}
										/>
