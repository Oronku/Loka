...
export function generateTripNotifications(trip: Trip): TripNotification[] {
  ...
  trip.flights.forEach((flight, index) => {
    ...
    if (checkinTime > now) {
      notifications.push({
        ...
        message: `Your flight ${flight.flightNumber} departs in 24 hours. Check in now to get a better seat!`,
        priority: 'high',
    ...
    if (!flight.checkedBag && hoursUntilFlight > 0 && hoursUntilFlight <= 48) {
      notifications.push({
        id: `flight-no-bag-${trip.id}-${index}`,
        tripId: trip.id,
        type: 'flight-no-baggage',
        title: '🎒 No Checked Baggage',
        message: `Your flight ${flight.flightNumber} has no checked baggage. Pack light and follow carry-on restrictions!`,
        priority: 'medium',
    ...
    if (arrivalReminderTime > now) {
      notifications.push({
        ...
        message: `Your flight ${flight.flightNumber} departs at ${departureTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}. ${isInternational ? 'International flight - arrive 3 hours early!' : 'Arrive 2 hours early for domestic flights.'}`,
        priority: 'critical',
    ...
    if (departureReminderTime > now) {
      notifications.push({
        ...
        message: `Flight ${flight.flightNumber} from ${flight.departureAirportCode} to ${flight.arrivalAirportCode} departs in 3 hours!`,
        priority: 'high',
  ...
  trip.attractions.forEach((attraction, index) => {
    ...
    const attractionDateTime = new Date(
      attraction.scheduledDate + 'T' + (attraction.scheduledTime || '10:00:00')
    );
