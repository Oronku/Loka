...
export default function AgencyDashboard() {
  ...
  const loadAgencyData = async () => {
    try {
      ...
      setError(err.response?.data?.error || 'שגיאה בטעינת נתוני הסוכנות');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'שגיאה בעדכון סוכן');
    }
  };

  const handleRemoveAgent = async (agentId: string) => {
    if (!confirm('האם אתה בטוח שברצונך להסיר את הסוכן מהסוכנות?')) {
      return;
    }
    ...
    try {
      ...
      setError(err.response?.data?.error || 'שגיאה בהסרת סוכן');
    }
  };
  ...
  const handleSendInvitation = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'שגיאה בשליחת הזמנה');
    }
  };
  ...
  const handleCancelInvitation = async (invitationId: string) => {
    try {
      ...
      setError(err.response?.data?.error || 'שגיאה בביטול הזמנה');
    }
  };
      ...
      <Box sx={{ py: 4 }}>
        ...
        <Paper sx={{ mb: 3 }}>
          <Tabs value={selectedTab} onChange={(e, v) => setSelectedTab(v)}>
            <Tab label="סוכנים" />
            <Tab label="הזמנות ממתינות" />
            <Tab label="טיולים אחרונים" />
          </Tabs>
        </Paper>
                ...
                <TableBody>
                  {agents.map((agent) => (
                    <TableRow key={agent._id}>
                      ...
                      <TableCell>
                        {agent.isAdmin && (
                          <Chip label="אדמין כללי" size="small" color="error" />
                        )}
                        {agent.isAgencyAdmin && (
                          <Chip
                            label="אדמין סוכנות"
                  ...
                  {stats.recentTrips.map((trip) => (
                    <TableRow key={trip._id}>
                      ...
                      <TableCell>
                        <Chip
                          label={
                            trip.status === 'published' ? 'פורסם' : 'טיוטה'
                          }
                          size="small"
                          color={
